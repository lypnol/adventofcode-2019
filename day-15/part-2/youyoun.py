...
def oxyg_fill_time(map, oxyg_pos):
    all_coords = set()
    oxyg_coords = {oxyg_pos}
    for e in map:
        all_coords.add(e)
        all_coords = all_coords.union(map[e])
    minutes = 0
    while len(oxyg_coords) < len(all_coords):
        for e in oxyg_coords:
            oxyg_coords = oxyg_coords.union(map[e])
        minutes += 1
    return minutes
    ...
    def run(self, s):
        ...
        while prog.end == 0:
            ...
            if len(prog.outputs) == 1:
                ...
                if passed == 2:
                    break
        return oxyg_fill_time(robot.map, oxyg)
