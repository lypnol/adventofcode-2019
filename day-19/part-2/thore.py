...
class ThoreSubmission(SubmissionPy):
    def run(self, s):
        ...
        x, y = get_closest_fit(program)
        return 10_000 * x + y


def get_closest_fit(program):
    upper_border = get_upper_beam_border(program, x_start=100)

    for x_u, y_u in upper_border:
        if is_beam(program, x_u - (SQUARE_SIZE - 1), y_u + SQUARE_SIZE - 1):
            return x_u - (SQUARE_SIZE - 1), y_u


def is_beam(program, x, y):
    vm = VM(program)
    vm.add_input(x)
    vm.add_input(y)
    vm.run()
    return vm.get_output() == 1


def get_upper_beam_border(program, x_start=0, y_start=0):
    x, y = x_start, y_start
    yield (x, y)
    while True:
        x += 1
        while not is_beam(program, x, y):
            y += 1
        yield (x, y)
