...
class JrmyySubmission(SubmissionPy):
    ...
    def run(self, s: str) -> int:
        ...
        ints[0] = 2
        c, relative_base, joy_pos, paddle_x, current_opt, score = 0, 0, 0, 0, [], None
        while ints[c] != 99:
            ints, c, relative_base, opt = self._run_opcode(ints, joy_pos, c, relative_base)
            if opt is not None:
                current_opt.append(opt)
            if len(current_opt) == 3:
                x, y, tile = current_opt
                current_opt = []
                if (x, y) == (-1, 0):
                    score = tile
                else:
    ...
    def _run_opcode(self, l: List[int], ipt: int, c: int, relative_base: int) -> Tuple[List[int], int, int, int]:
        ...
        if op_code == 1 or op_code == 2:
            ...
            self._set_value_at_pos(l, position, ipt)
            next_cursor = c + 2
