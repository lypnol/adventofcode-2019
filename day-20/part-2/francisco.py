...
def solve_part2(lines):
    ...
    while True:
        ...
        if pos == (0, OUTSIDE, end):
            return distance

        (level, side, code) = pos

        # go through door
        other_level = level + (1 if side == INSIDE else -1)
        other_side = (side + 1) % 2
        if other_level >= 0 and (other_level, other_side, code) not in seen:
            heapq.heappush(q, (distance + 1, (other_level, other_side, code)))

        # regular move
        for (next_side, next_code), distance_pos in adj[(side, code)].items():
            if level > 0 and next_code in [start, end]:
                continue
            if (level, next_side, next_code) in seen:
                continue

            heapq.heappush(q, (distance + distance_pos, (level, next_side, next_code)))


class FranciscoSubmission(SubmissionPy):
    def run(self, s):
        return solve_part2(s.split("\n"))
