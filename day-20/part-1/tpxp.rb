class Solution
    ...
    @map.length.times do |y|
      @map[y].length.times do |x|
        ...
        if not @map[y][x+1] or to_skip.include? @map[y][x+1]
          next if not @map[y+1] or to_skip.include? @map[y+1][x]
          label += @map[y+1][x]
          [-1, 2].each do |dy|
  ...
  def run
    ...
    while @to_explore.length > 0
      ...
      d = @dist[pos] + 1
      [-1,1].each do |dx|
        r = explore_pos d, x+dx, y, dx, 0
        return r if r
      end
      [-1,1].each do |dy|
        r = explore_pos d, x, y+dy, 0, dy
        return r if r
      end
    end
  end

  def explore_pos(d, x,y, dx, dy)
    return if x < 0 or y < 0
    return unless @map[y] and @map[y][x]
    return if @map[y][x] == '#'
    if @map[y][x] == '.'
      return if @dist[[y,x]] <= d
      @dist[[y,x]] = d
      @to_explore.push [y,x]
    else
      # Portal
