...
class Explorer
  attr_reader :dx, :dy, :px, :py, :keys

  def inspect
    "Explorer<px=#{@px},py=#{@py},dx=#{@dx},dy=#{@dy},#{@keys.length} keys>"
  end

  def initialize(map, px, py, dx, dy, keys)
    @map = map
    @px = px
    @py = py
    @dx = dx
  ...
  def ==(other)
    other.dx == @dx and other.dy == @dy and other.py == @py and other.px == @px and other.keys == @keys
  end

  def explore
    return -1 if (@py < 0 or @py >= @map.length or @px < 0 or @px >= @map[@py].length or @map[@py][@px] == '#')
    # Catch the key on the current tile if possible
    unless %w(. @).include? @map[@py][@px]
      l = @map[@py][@px]
      if l.upcase == l
        return -1 unless @keys.include? l.downcase
      end
      @keys.add l
    end
    throw :gotAll if @keys.length == 26 # It also has '@'
    # If possible create explorers around -but don't go backwards!
    res = []
    if @dx == 0
      [-1,1].each do |dx|
        next if @px + dx < 0 or @px + dx >= @map[@py].length
        next if @map[@py][@px+dx] == '#'
        res.push Explorer.new @map, @px+dx, @py, dx, 0, @keys
      end
    end
    if @dy == 0
      [-1,1].each do |dy|
        next if @py + dy < 0 or @py + dy >= @map.length
        next if @map[@py+dy][@px] == '#'
        res.push Explorer.new @map, @px, @py+dy, 0, dy, @keys
      end
    end
    # Move on
    @py += @dy
